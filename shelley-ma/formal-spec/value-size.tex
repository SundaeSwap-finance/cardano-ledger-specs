...
Two types are used to define concrete ledgers, $\Coin$ and $\Value_C$.
...
The function $\fun{serSize}$, unlike the functions $\fun{size}$ and $\fun{utxoEntrySize}$
    ...
    of $\Value_C$ terms in outputs), whereas the min-Ada requirement is about
...
The $\fun{size}$ function returns the estimated size of a $\Value_C$ term. The size
...
the size of a UTxO entry, not counting the size of the $\Value_C$ term it contains.
Here, again, the actual size of a UTxO (excluding the $\Value_C$) can vary, but
...
UTxO. It adds the size estimate of the $\Value_C$ term in a UTxO and the
