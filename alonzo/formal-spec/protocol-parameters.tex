...
\begin{tabular}{lp{5in}}
  ...
  \begin{note}
    Add a link to $\CostMod$ documentation
  \end{note}
  \\
  $\Language$ &
  ...
  \\
  $\Prices$ &
  A term of this type comprises two rational number values that correspond to the components of $\ExUnits$,
...
The functions $\fun{serialize}$ and $\fun{serializeEncoding}$ are left abstract, as
these are implementation-dependent serialization functions. They must
be implemented for all types to which they are applied, including all
...
the language tag, and the serialized representation of the relevant PPs.
...
\begin{figure*}[htb]
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}l@{\qquad\qquad\qquad}r}
      \fun{serialize},~\fun{serializeEncoding} & \mathsf{Type} & \ByteString \\
       \text{Serialization functions}
    \end{array}
  \end{equation*}
  %
  \emph{Abstract types}
  ...
  \begin{align*}
    ...
    & \fun{getLanguageView}~\var{pp}~\PlutusVI = (\fun{serialize}~\PlutusVI, ~ (\fun{serializeEncoding}~(\fun{costmdls}~{pp}~\{\PlutusVI\})))
  \end{align*}
